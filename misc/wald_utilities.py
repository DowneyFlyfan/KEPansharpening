...
def interp23tap(img, ratio):
    ...
    assert (
        2 ** (round(math.log(ratio, 2)))
    ) == ratio, "Error: Only resize factors power of 2"
...
def wald_protocol(ms, pan, ratio, sensor, channels=8):
    ...
    for i in range(ms_wald_.shape[0]):
        temp = np.copy(
            np.asarray(
                torch.squeeze(torch.squeeze(ms_wald_[i, :, :, :]).permute((1, 2, 0)))
                .detach()
                .cpu()
            )
        )
        ms_lr[i, :, :, :] = torch.from_numpy(interp23tap_GPU(temp, ratio)).permute(
            (2, 0, 1)
        )
    ...
    def genMTF_MS():
        ...
        for i in range(ms_wald_.shape[0]):
            temp = np.copy(
                np.asarray(
                    torch.squeeze(
                        torch.squeeze(ms_wald_[i, :, :, :]).permute((1, 2, 0))
                    )
                    .detach()
                    .cpu()
                )
...
def MTF(ms, sensor, ratio, channels):
    ...
    MTF_kern = np.moveaxis(mtf_kernel, -1, 0)
    MTF_kern = np.expand_dims(MTF_kern, axis=1)
    MTF_kern = torch.from_numpy(MTF_kern)
    ...
    depthconv.weight.data = MTF_kern.to(bargs.device, bargs._dtype)
    depthconv.weight.requires_grad = False
    ms_lr = depthconv(ms)
